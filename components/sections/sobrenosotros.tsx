...
export function SobreNosotros() {
    ...
    useEffect(() => {
        ...
        const SCROLL_SPEED = 0.5 // píxeles por frame (ajusta para más rápido/lento)
        ...
        const animate = (currentTime: number) => {
            ...
            if (!isPaused && !isUserScrollingRef.current && isVisible) {
                // Avance basado en tiempo para que sea consistente entre dispositivos
                container.scrollTop += SCROLL_SPEED * (deltaTime / 16.67)

                // Reset al llegar a la mitad (porque duplicamos las imágenes)
    ...
    // Detectar scroll manual del usuario para pausar momentáneamente
    const handleUserScroll = () => {
        ...
        // Reanuda la animación 1.5s después de que el usuario deja de scrollear
        userScrollTimeoutRef.current = setTimeout(() => {
            isUserScrollingRef.current = false
        }, 1500)

        // Mantener el loop infinito incluso con scroll manual
        const container = scrollContainerRef.current
        if (container) {
            const halfHeight = container.scrollHeight / 2
            if (container.scrollTop >= halfHeight) {
    ...
    return (
        ...
        >
            ...
            <div className="absolute inset-0 opacity-5">
                ...
                <div className="absolute bottom-0 right-0 w-72 h-72 bg-primary rounded-full blur-3xl" />
            </div>
            ...
            <div className="container mx-auto px-4 relative z-10">
                ...
                <div className="grid lg:grid-cols-2 gap-12 items-center">
                    ...
                    <div className="grid grid-cols-2 gap-6 order-2 lg:order-1">
                        {[
                            ...
                            <motion.div
                                ...
                                className="bg-card border border-border rounded-2xl p-6 text-center shadow-md"
                            >
                    ...
                    {/* Galería vertical con auto-scroll */}
                    <div className="relative order-1 lg:order-2">
                        {/* Gradiente superior - indica que hay más arriba */}
                        <div className="absolute top-0 left-0 right-0 h-20 bg-gradient-to-b from-muted/30 via-muted/20 to-transparent z-20 pointer-events-none" />

                        {/* Gradiente inferior - indica que hay más abajo */}
                        <div className="absolute bottom-0 left-0 right-0 h-20 bg-gradient-to-t from-muted/30 via-muted/20 to-transparent z-20 pointer-events-none" />
                        ...
                        >
                            <div className="flex flex-col gap-6 px-2">
                                {duplicatedImages.map((item, index) => (
                                    <div
                                        key={`${item.id}-${index}`}
                                        className="group relative overflow-hidden rounded-3xl shadow-xl w-full h-[280px] md:h-[350px] lg:h-[400px] flex-shrink-0"
                                    >
                                        <img
                                            src={item.image}
